...
/*MAKE LEET*/
/*This program takes a text file, translates it
 to "leet" and prints the result as well as wordcount of file.*/
...
int * wordCount(FILE *fileptr) {
  ...
  int *countarray = (int *) malloc(3 * sizeof(wordcount));
 ...
 */
void replaceChars(FILE *fileptr, int * result) {
  char curr_ch = getc(fileptr);  // The current character in the file
  int newstr_index = 0;  // The next index in the new string
  char * newString = (char *) malloc(*(result+2) * sizeof(curr_ch));
